...
class TECNAS:
    ...
    def train_model(self, arch_obj, epochs = EPOCHS):
        ast = self.ast
        print(Fore.BLUE + f'\n{ast} EXECUTION {self.exec+1}/{EXECUTIONS} GENERATION {self.generation+1}/{GENERATIONS} {crossover_type} {mutation_type} {self.arch_count+1}/{TOTAL_ARCH} architectures {ast}\n')
        self.arch_count += 1
        ...
        if arch_obj.idx in list(self.trained_archs.keys()):
                ...
                self.reporter.save_arch_info(arch, self.search_strategy, self.crossover_type, self.mutation_type, self.generation, self.exec, self.local_seed, self.succ_cross, self.succ_mut, self.total_cross, self.total_mut)
        ...
        if TRAIN == False:
            ...
            self.reporter.save_arch_info(arch_obj, self.search_strategy, self.crossover_type, self.mutation_type, self.generation, self.exec, self.local_seed, self.succ_cross, self.succ_mut, self.total_cross, self.total_mut)
                ...
                for mutation_type in mutation_types_list:
                    ...
                    for e in range(EXECUTIONS):
                        ...
                        for g in range(GENERATIONS):
                            ...
                            reporter.save_arch_info(sorted_pop[0], self.search_strategy, self.crossover_type, self.mutation_type, self.generation, self.exec, -9999, -9999, -9999, -9999, -9999)
