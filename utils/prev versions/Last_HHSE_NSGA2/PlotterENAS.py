...
class Plotter:
    ...
    def get_all_medians_folder(self): 
        ...
        for archs_file in os.listdir(folder):
            if archs_file.endswith(".csv"):
                if is_HHSE(archs_file):
                    print(f'Skipping {archs_file} since it is HHSE')
                    continue
        ...
        for column in columns:
            ...
            for filename in os.listdir(self.splitted_folder):
                if filename.endswith(".csv"):
                    ...
                    labels.append(filename[:-4])  # Remove the .csv extension from the filename
    ...
    def plot_measures_from_folder(self, plot_bestarchs = True):
        ...
        for column in columns:
            ...
            for filename in os.listdir(self.to_be_plotted_folder):
                if filename.endswith(".csv"):
                    print(f'\nProcessing {filename}')
                    if plot_bestarchs == False and (column in self.columns_GA_avoid_NONE) and 'NONE' in filename:
                        print(f'Skipping {filename} for {column} since it contains NONE strategy')
                        continue
            ...
            if np.nan in [min_val, max_val]: #If value is not empty, set the limits check this
    ...
    def plot_convergence_exec(self):
        ...
        for filename in os.listdir(folder):
            if filename.endswith(".csv"):
                if is_HHSE(filename):
                    print(f'Skipping {filename} since it is HHSE')
                    continue
    ...
    def __init__(self, experiment_folder):
        ...
        self.columns_GA_avoid_NONE = ['HD_P1', 'HD_P2', 'HD_BM']
