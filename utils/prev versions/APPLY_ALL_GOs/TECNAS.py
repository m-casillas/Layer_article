...
class TECNAS:
    ...
    def apply_all_GOs(self): #==================================================APPLY ALL GOs=======================================================
        ...
        for crossover_type in config_tecnas.crossoverList_hhse: #['SPC', 'TPC', 'UC']
            self.crossover_type = crossover_type
            for mutation_type in config_tecnas.mutationList_hhse: #['MPAR', 'MSWAP']
                ...
                print('Applying all genetic operators in course. Selecting next generation')
                self.GOs_population_dict[cross_mut_str] = copy.deepcopy(self.selection(pop = self.pool))
                print('Selection of next generation complete\n')
                self.GOs_HV[cross_mut_str] = self.HV
                statusObj = Status(tecnasObj = self, crosstype = crossover_type, muttype = mutation_type, pop = self.GOs_population_dict[cross_mut_str])
                self.reporter.save_generation_status(statusObj)
                self.generation_status[cross_mut_str] = statusObj
        best_go = self.select_best_GOs()[0]#Because it returns a list with one element (the selected GO as string)
        ...
        self.generation_status[best_go].GOs_history = self.GOs_history
        self.generation_status[best_go].selected_by = self.selected_by
        #Report the best_go status
    ...
    def select_best_GOs(self): #==================================================SELECT BEST GOs=======================================================
        def check_GoCandidates():
            finish = False
            self.GOcandidates = self.GOcandidates if len(self.GOcandidates) > 0 else self.GOcandidatesAux
            finish = True if len(self.GOcandidates) == 1 else False
            return finish

        def select_criteria(criteria = ''):
            print(f'Before {criteria = }')
            print(f'{self.GOcandidates = } {self.GOcandidatesAux = }')
            for go_name in self.GOcandidatesAux:
                print(f'{self.generation_status[go_name]}')
            print()
            if criteria == 'dPB':
                self.GOcandidates = [go_name for go_name in self.GOcandidates if self.generation_status[go_name].dPB > 0]
            elif criteria == 'dHV':
                self.GOcandidates = [go_name for go_name in self.GOcandidatesAux if self.generation_status[go_name].dHV > 0]
            elif criteria == 'succ_cross':
                self.GOcandidates = [go_name for go_name in self.GOcandidates if self.generation_status[go_name].succ_cross >= 0.30]
            elif criteria == 'succ_mut':
                self.GOcandidates = [go_name for go_name in self.GOcandidates  if self.generation_status[go_name].succ_mut >= 0.30]
            else:
                print(f'ERROR: {criteria} selection criteria not recognized. Returning None')
                return None

            print(f'Number of candidates after {criteria}: {len(self.GOcandidates)}')
            finish = check_GoCandidates()
            self.GOcandidatesAux = copy.copy(self.GOcandidates) 
            if finish:
                self.selected_by = criteria

            print(f'After {criteria = }')
            print(f'{self.GOcandidates = } {self.GOcandidatesAux = }\n')
            self.GOs_history[criteria] = self.GOcandidates.copy()
             
            
            return finish

        self.selected_by = ''
        self.GOnames_list = list(self.GOs_names)
        self.GOcandidates = []
        self.GOcandidatesAux = copy.copy(self.GOnames_list)
        self.GOs_history = {} #{'dPB': ['SPC_MPAR', 'TPC_MSWAP], 'dHV': 'UC_MPAR', 'TPC_MPAR',...} GOs that passed the criteria

        finish = select_criteria(criteria='dHV')
        if finish:
            return self.GOcandidates
        finish = select_criteria(criteria='dPB')
        if finish:
            return self.GOcandidates
        finish = select_criteria(criteria='succ_cross')
        if finish:
            return self.GOcandidates
        finish = select_criteria(criteria='succ_mut')
        if finish:
            return self.GOcandidates

        #best_go = max(self.GOcandidatesAux, key=lambda go: self.generation_status[go].best_accuracy)
        best_go = max(self.GOcandidatesAux, key=lambda go: self.generation_status[go].HV)
        self.GOcandidates = [best_go]
        self.selected_by = 'best_HV'
        self.generation_status[best_go].GOs_history[self.selected_by] = self.GOcandidates
    ...
    def __init__(self, HHSE = False, NSGA2 = False, report_training = True, representation_type = ConfigClass.REPRESENTATION_TYPE, encoding_type = ConfigClass.ENCODING_TYPE, experiment_folder = '', regressor_type = -1):
        ...
        #For HHSE ========================================= HHSE ========================================================================
        self.GOs_names = [f"{cross}_{mut}" for cross, mut in product(self.crossover_types_list, self.mutation_types_list)] #['SPC_MPAR', 'SPC_MSWAP', 'TPC_MPAR', 'TPC_MSWAP', 'UC_MPAR', 'UC_MSWAP',...]
