...
def split_all_csvs_in_folder(input_folder, path_results):
    ...
    for input_file in os.listdir(input_folder):
        ...
        if input_file.endswith('.csv'):
            ...
            if df['HHSE'].any():
                print('HHSE detected')
                hhse_types = df['HHSE_TYPE'].unique()
                for hhse_type in hhse_types:
                    hhse_subset = df[df['HHSE_TYPE'] == hhse_type]
                    filename = f'HHSE_{hhse_type}'
                    filename += '_NSGA2.csv' if hhse_subset['NSGA2'].any() else '.csv'
                    custom_folder = ''
                    custom_dir = os.path.join(path_results, custom_folder)
                    ensure_folder_exists(custom_dir)
                    filepath = os.path.join(custom_dir, filename)
                    hhse_subset.to_csv(filepath, index=False)
