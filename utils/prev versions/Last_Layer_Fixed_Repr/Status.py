...
class Status:
    def __init__(self, tecnasObj = None, crosstype = '', muttype = '', pop = None, JUST_PARETO = False):
        ...
        if JUST_PARETO: #Save information of the architectures in the Pareto front, each generation.
            self.HV = self.tecnasObj.HV
            self.dHV = self.tecnasObj.dHV
        else:
            self.HV = self.tecnasObj.GOs_HV[self.GO_name]
            self.dHV = 0 if self.generation == 1 else self.HV - self.tecnasObj.GOs_prevHV[self.GO_name]
