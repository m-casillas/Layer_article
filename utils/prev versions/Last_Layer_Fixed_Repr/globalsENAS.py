...
def reset_columns_dict(dictionary, reset_columns):
    ...
    for col in reset_columns:
        if col in dictionary:
            dictionary[col] = np.nan
    return dictionary

def delete_columns_list(df, cols):
    #Deletes the columns with null or negative values in the dataframe
    return  [c for c in cols if ((df[c] >= 0).any())]

def is_HHSE_NSGA2(filename):
    return (True if 'HHSE' in filename else False, True if 'NSGA2' in filename else False)
...
def remove_duplicates(df, column_name='Integer_encoding', metric='Accuracy', keep='max'):
    #Removes duplicate rows in a DataFrame based on a specified column and max, min or first occurence of a metric.
    if column_name not in df.columns or metric not in df.columns:
        raise ValueError(f"The DataFrame must contain '{column_name}' and '{metric}' columns.")
    duplicated_mask = df.duplicated(subset=column_name, keep=False)
    duplicated_rows = df[duplicated_mask]
    unique_rows = df[~duplicated_mask]
    detected_duplicates = len(duplicated_rows)
    if keep == 'max':
        resolved_duplicates = (duplicated_rows.sort_values(by=metric, ascending=False).drop_duplicates(subset=column_name, keep='first'))
    elif keep == 'min':
        resolved_duplicates = (duplicated_rows.sort_values(by=metric, ascending=True).drop_duplicates(subset=column_name, keep='first'))
    else:
        resolved_duplicates = duplicated_rows.drop_duplicates(subset=column_name, keep=keep)
    eliminated_duplicates = detected_duplicates - len(resolved_duplicates)

    final_df = pd.concat([unique_rows, resolved_duplicates])
    print(Fore.YELLOW + f"Eliminated duplicate rows: {eliminated_duplicates}" + Style.RESET_ALL)
    print(Fore.YELLOW + f"Final entries after resolving duplicates based on {column_name}: {len(final_df)} " + Style.RESET_ALL)
    return final_df

def print1(*args, verbose = config_tecnas.PRINT1): #Most important prints
    if verbose: print(*args)

def print2(*args, verbose = config_tecnas.PRINT2): #Less important prints
    if verbose: print(*args)

def print3(*args, verbose = config_tecnas.PRINT3): #Debug prints
    if verbose: print(*args)
...
def filter_csv(df_folder_path = '', df = None, generation = 'ALL', arch_status = 'BEST', rank = 'ALL', nparts = 1) :
    def filter_df(df):
        ...
        if rank != 'ALL': #Keep architectures with ranking rank. Only one architecture per combination of crossover and mutation
            ...
            if rank == 'HIGHEST':
                if 'HHSE' in df.columns and df['HHSE'].any(): #If this is a HHSE experiment, get the biggest of all rank 1 architectures
...
def ensure_folder_exists(folder_path):
    if not os.path.exists(folder_path):
        ...
        print2(f"Folder created at: {folder_path}")
    else:
        print2(f"Folder already exists at: {folder_path}")
...
def count_flops_analytic(arch, input_res=32):
    #arch is Genotype as string
    arch = flat_str(arch)
...
def count_params_analytic(arch, bytes_per_param=4):
    arch = flat_str(arch)
    arch = ast.literal_eval(arch)
    C_in = 3
    H = W = None
...
class ConfigClass:
    ...
    MUT_PROB = 1/LAYERS_CONSTANTS.NUM_MUTABLE_LAYERS if REPRESENTATION_TYPE == 'L' else 1/BLOCKS_CONSTANTS.SIZE_BLOCKGEN
