...
class TECNAS_NSGAII:
    ...
    def evaluate(self, archs):
        ...
        for i,arch in enumerate(archs):
            if arch.flops > 0:
                ...
                self.tecnas.log_out_of_bounds_arch(arch)
    ...
    def _build_objective_matrix(self, archs):
        return np.column_stack([
             np.array([1.0 - a.acc for a in archs]),  # Maximize accuracy -> minimize (1 - acc)
    ...
    def normalize_objectives(self, raw_objectives):
        ...
        normalized = raw_objectives.copy()
        min_flops = self.tecnas.MINFLOPS
        max_flops = self.tecnas.MAXFLOPS
        min_params = self.tecnas.MINPARAMS
        max_params = self.tecnas.MAXPARAMS
        ...
        normalized[:,1] = (raw_objectives[:,1] - min_flops) / flops_range
        normalized[:,2] = (raw_objectives[:,2] - min_params) / params_range
    ...
    def select_and_HV(self, archs):
        ...
        self.HV = self.hypervolume(F, fronts, ref_point=None)
        print('Calculating HyperVolume complete')
        selected_indices = []
        for front in fronts:
        ...
        return [archs[i] for i in selected_indices], self.HV
    ...
    def plot_pareto_front_3D(self, F, fronts):
        ...
        F_plot = F.copy()
        F_plot[:, 0] = 1.0 - F_plot[:, 0] #F_plot[:, 0] = -F_plot[:, 0]
        ...
        ax.set_ylabel('MIN FLOPs 10^8')
        ax.set_zlabel('MIN NParams 10^6 (minimized)', labelpad=15)
        ax.set_title(f"Pareto Front — Exec {self.tecnas.exec},Gen {self.tecnas.generation}\n{self.tecnas.crossover_type}_{self.tecnas.mutation_type} HV: {self.HV:.4f}")
        ax.legend()
        # Rotate view
