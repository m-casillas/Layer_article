...
class TECNAS:
    ...
    def report_op_counts(self):
        if config_tecnas.HHSE_GREEDY == False:
            return
        folder = ensure_folder_exists('Pareto_Fronts')
        file_path = os.path.join(folder, f"Op_count_Exec{self.exec}.txt")
...
# ================================================================================= MARKOV===================================================================
        ...
        for crossover_type in config_tecnas.crossoverList_hhse: #['SPC', 'TPC', 'UC']
            ...
            for mutation_type in config_tecnas.mutationList_hhse: #['MPAR', 'MSWAP']
                ...
                if config_tecnas.IGNORE_RANDOM and self.crossover_type == 'NONE' and self.mutation_type == 'NONE':
                    continue
                self.selected_by = ''
                print2(f'{crossover_type}_{mutation_type} COMBINATION\n')
                ...
                self.GOs_prevHV[cross_mut_str] = self.prev_HV
                self.reporter.save_generation_status(statusObj)
                self.generation_status[cross_mut_str] = statusObj
                
        best_go = self.select_best_GOs(criteria = config_tecnas.HHSE_GREEDY_CRITERIA)
    ...
    def select_best_GOs(self, criteria = ''): #==================================================SELECT BEST GOs=======================================================
        ...
        if criteria == 'HV':
            ...
            best_go = max(self.GOs_names, key = lambda go: self.generation_status[go].best_accuracy)

        else:
            best_go = None
        
        self.GOs_history.append(best_go)  
        self.selected_by = f'{criteria}'
    ...
    def validate_architecture(self, gen_list, arch_idx = ''):
        #[{'INP': 32}, {'POOLMAX': [-1, 3]}, {'POOLMAX': [-1, 3]}, {'CONV': [256, 3]}, {'POOLMAX': [-1, 2]}, {'POOLMAX': [-1, 2]}, {'CONV': [32, 3]}, {'FLATTEN': None}, {'DENSE': [128, 'relu']}, {'LAST_DENSE': [10, 'softmax']}]
        #Determine if the architecture is valid before training. Check that there are no consecutive POOL layers
        #A valid layer architecture should start with an input, followed by a CONV. After FLATTEN, there should be a DENSE, followed by a LAST_DENSE.
        #No consecutive POOLs are allowed.
        #Max number of pools is given by GLOBALS.LAYERS_CONSTANTS.MAX_POOLS
    ...
    def random_genlist(self, num_dense = ConfigClass.DATASET_NUMDENSE_DICT):
        #[{'INP': 32}, {'CONV': [128, 5]}, {'CONV': [128, 5]}, {'CONV': [64, 3]}, {'POOLMAX': [-1, 3]}, {'CONV': [256, 5]}, {'CONV': [256, 3]}, {'FLATTEN': None}, {'DENSE': [512, 'relu']}, {'LAST_DENSE': [10, 'softmax']}]
        #[5, 5, 2, 9, 7, 6, 12]
        gen_list = []
        if self.representation_type == 'L':
            pool_count = 0
            max_pool_count = LAYERS_CONSTANTS.MAX_POOLS
    ...
    def train_model(self, arch = None, calculate_cm = False, epochs = ConfigClass.EPOCHS, print_status = True):
        ...
        if self.generation > 0:
            if print_status == True:
                ...
                hhse_str = 'RANDOM' if config_tecnas.HHSE_RANDOM == True else f'MARKOV {self.markov_type} {"COMBINED" if self.COMBINED_MARKOV else ""}' if config_tecnas.HHSE_MARKOV == True else f'GREEDY: {config_tecnas.HHSE_GREEDY_CRITERIA}' if config_tecnas.HHSE_GREEDY == True else ''
    ...
    def HHSE_MARKOV(self):
        perc_intervals = config_tecnas.perc_intervals
        total_gen = config_tecnas.GENERATIONS
        limits = []
        cumulative = 0
        for p in perc_intervals:
            cumulative += p / 100
            limits.append(cumulative)
        progress = self.generation / total_gen

        if self.COMBINED_MARKOV:
            if progress <= limits[0]:
                self.markov_type = 'MUT'
            elif progress <= limits[1]:
                self.markov_type = 'HV'
            elif progress <= limits[2]:
                self.markov_type = 'DHV'
            elif progress <= limits[3]:
    ...
    def ENAS(self):
        ...
        for e in range(1,ConfigClass.EXECUTIONS+1):
            ...
            self.op_count = {}
            self.GOs_history = []
            self.current_state = 'SPC_MPAR' if config_tecnas.ENCODING_TYPE == 'INT' else 'SPC_MBFLIP' #For MARKOV
        ...
        if config_tecnas.HHSE_GREEDY == True:
    ...
    def __init__(self, HHSE = False, NSGA2 = False, report_training = True, representation_type = ConfigClass.REPRESENTATION_TYPE, encoding_type = ConfigClass.ENCODING_TYPE, experiment_folder = '', regressor_type = -1):
        ...
        self.BLOCKS_MAXFLOPS  = 1.53e10
        self.BLOCKS_MINPARAMS = 2.0e5
        self.BLOCKS_MAXPARAMS = 2.50e7
        ...
        self.dHV = 0
        self.GOs_prevHV = {} #For storing previous HV obtained after applying each GO, for HHSE GREEDY
