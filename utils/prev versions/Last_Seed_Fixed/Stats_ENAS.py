...
def correlation_matrix_folder(experiment_folder = ''):
    ...
    for filename in os.listdir(splitted_folder):
        ...
        print(f"Correlation matrix {file}...")
        df = pd.read_csv(file)

        dfBestGen = df[df['arch_status'] == 'BEST']
        candidate_cols = (config_tecnas.plot_archcolumns + config_tecnas.plot_GAcolumns)

        # 1. Keep only columns that actually exist in the dataframe
        existing_cols = [c for c in candidate_cols if c in dfBestGen.columns]
        selected_cols = dfBestGen[existing_cols]
        # 2. Drop columns with any NaN values
        selected_cols = selected_cols.dropna(axis=1)
        # 3. Drop columns that contain any negative values
        selected_cols = selected_cols.loc[:, (selected_cols >= 0).all()]

        corr_matrix = selected_cols.corr(method = 'spearman')
...
def correlation_matrix_execution(experiment_folder = ''):
    splitted_folder = os.path.join(experiment_folder, 'splitted')
    ensure_folder_exists(splitted_folder)
    figs = []
    for filename in os.listdir(splitted_folder):
        file = os.path.join(splitted_folder, filename)
        correlation_plot_folder = os.path.join(experiment_folder, 'stats', 'correlation', f'{filename[:-4]}_per_execution')
        ensure_folder_exists(correlation_plot_folder)
        print(f"Correlation matrix {file}...")
        df = pd.read_csv(file)
        executions = df['Execution'].unique().tolist()
        for exec in executions:
            print(f'Correlation matrix for {filename} execution {exec}...')
            dfexec = df[df['Execution'] == exec]
            dfBestGen = dfexec[dfexec['arch_status'] == 'BEST']
            candidate_cols = (config_tecnas.plot_archcolumns + config_tecnas.plot_GAcolumns)
            # 1. Keep only columns that actually exist in the dataframe
            existing_cols = [c for c in candidate_cols if c in dfBestGen.columns]
            selected_cols = dfBestGen[existing_cols]
            ...
            fig = plt.figure(figsize=(8, 6))
            sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt='.2f', vmin=-1, vmax=1)
            plt.title(f"Correlation Matrix. Execution {exec}\n{filename[:-4]}")
            plt.tight_layout()
            out_path = os.path.join(correlation_plot_folder, f'{filename[:-4]}_exec{exec}_correlation_matrix.png')
            plt.savefig(out_path, dpi=200)
            figs.append(fig)
            plt.close(fig)
