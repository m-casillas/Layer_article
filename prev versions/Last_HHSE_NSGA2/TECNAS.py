...
'''
from TECNAS_NSGAII import TECNAS_NSGAII, GeneticOperator
from TECNAS_NSGAII_LOGGER import TECNAS_NSGAII_LOGGER
...
class TECNAS:
    ...
    def flops_params_gen_list_str(self, getflops = True, get_numparams = True, gen_list_str = None):
        #Returns FLOPs and NumParams for a given gen_list
        import time
        memory = psutil.virtual_memory()
        print(Fore.RED + f"FLOPs Params calculation: RAM used: {memory.percent}%")
        arch_obj = self.create_arch_from_genlist(ast.literal_eval(gen_list_str))
        flops =  calculate_model_flops(arch_obj.model) if getflops else 0
        num_params, sizeMB = calculate_model_params(arch_obj.model) if get_numparams else (0,0)
        print(Fore.RED + f"FLOPs Params calculation complete")
    ...
    def apply_all_GOs(self):
        ...
        for crossover_type in config_tecnas.crossoverList_hhse:
            ...
            for mutation_type in config_tecnas.mutationList_hhse:
                ...
                #evaluate_best_children(self, tecnas = None, children = None, objective_names = None, objective_maxmin_names = None, flops_params_arch_func = None)
                self.nsga2.genetic_operators_obj_dict[cross_mut_str].evaluate_best_children(tecnas = self, children = copy.deepcopy(self.children), objective_names=self.objective_names, objective_maxmin_names=self.objective_maxmin_names, flops_params_gen_list_str_func = self.flops_params_gen_list_str)
    ...
    def train_model(self, arch = None, calculate_cm = False, epochs = ConfigClass.EPOCHS, print_status = True):
        ...
        if print_status == True:
            ...
            print(Fore.LIGHTBLUE_EX + f'\n{ast_bar} EXECUTION {self.exec}/{ConfigClass.EXECUTIONS} GENERATION {self.generation}/{ConfigClass.GENERATIONS}{ast_bar}')
    ...
    def set_arch_statuses(self):
        ...
        self.get_NFHT(self.sorted_pool[0])
    ...
    def ENAS(self):
        ...
        if ConfigClass.RANDOMIZE_SEED == True:
            ...
            self.local_seed = config_tecnas.SEED_LIST[0]
        ...
        for e in range(1,ConfigClass.EXECUTIONS+1):
            ...
            for g in range(1,ConfigClass.GENERATIONS+1):
                ...
                if self.HHSE == True:
                    ...
                    print(Fore.MAGENTA + 'Hyper-heuristic selection of genetic operators' + Fore.RESET)
                    self.apply_all_GOs()
                    best_GOs_str = self.nsga2.run_nsga2()
                    print(f'{best_GOs_str = }') 
                    self.crossover_type, self.mutation_type = best_GOs_str.split('_')
                    print(Fore.MAGENTA + f'Crossover: {self.crossover_type}, Mutation: {self.mutation_type}' + Fore.RESET)
        ...
        if self.local_seed == 58:
            return

    def __init__(self, HHSE = False, report_training = True, representation_type = ConfigClass.REPRESENTATION_TYPE, encoding_type = ConfigClass.ENCODING_TYPE, experiment_folder = '', regressor_type = -1):
        ...
        #Save here all children generated through all combinations of crossover and mutation
        #For example, = {'SPC_MPAR': [child1, child2, ...], 'SPC_MSWAP': [child1, child2, ...], ...}
